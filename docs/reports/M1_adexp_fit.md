...
**状态**：骨架（数值由 `scripts/make_figures.py` 与 `fre fit` / `pytest` 填充）  
...
- 产物：`artifacts/adexp.npz` + `adexp.meta.json`（含 `type_id`、`layer`、`git_commit`、`config_hash`）

## 结果摘要

| 指标 | 目标 | 实测 |
|---|---|---|
| \(R^2\) | ≥ 0.98 | （运行 `fre fit --model adexp` 后填入） |
| MSE | 报告 | |
| 方法 | pchip / cubic | |
