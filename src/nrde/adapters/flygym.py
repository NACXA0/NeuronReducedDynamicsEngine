...
from nrde.adapters.embodied import EmbodiedEnv
from nrde.engine.rate import step_rate
from nrde.engine.spike import init_spike_state, step_spike_lut
from nrde.types import FittedActivation, GraphData, RateState, SpikeState
...
class NRDEFlyGymEnv(EmbodiedEnv):
...
def maybe_make_flygym_sim() -> Any:
    """Return FlyGym Simulation class, or Mock when the optional extra is missing."""
    try:
...
def require_flygym() -> Any:
    """Import flygym or raise with an install hint (NFR-8 / RFC-002 §2.2)."""
    try:
        import flygym  # type: ignore

        return flygym
    except ImportError as exc:
        raise ImportError(
            "FlyGym is an optional extra. Install with: "
            "pip install 'neuron-reduced-dynamics-engine[flygym]'"
        ) from exc
